...
import { eventsModel, Isession } from '../shared/export-import';
...
export class EventServiceService {
  ...
  getEvents(): Observable<eventsModel[]> {
    let copyList = this.http.get<eventsModel[]>('/api/events').pipe(catchError(this.handleError<eventsModel[]>('getEvents', [])));
    // this.getEventList(copyList);


    return this.http.get<eventsModel[]>('/api/events').pipe(catchError(this.handleError<eventsModel[]>('getEvents', [])));
  ...
  getEvent(eventId): Observable<eventsModel> {
    console.log(eventId)
    return this.http.get<eventsModel>('/api/events/' + eventId).pipe(catchError(this.handleError<eventsModel>('getEvent')));
  ...
  saveEvent(eventValues) {
    ...
    return this.http.post<eventsModel>('/api/events', eventValues, options).pipe(catchError(this.handleError<eventsModel>('SaveEvent')));
  }

  updateEvent(event) {
    let index = events.findIndex(index => index.id = event.id);
    events[index] = event;

  }

  searchSessions(searchTerm: string): Observable<Isession[]> {
    return this.http.get<Isession[]>('/api/sessions/search?search=' + searchTerm).pipe(catchError(this.handleError<Isession[]>('seachSessions')));
...
const events: eventsModel[] = [
  ...
  },
  {
    ...
    location: {
      address: 'Convention Center & Scuba Shop',
      city: 'Amsterdam',
      country: 'Netherlands'
    },
    sessions: [
